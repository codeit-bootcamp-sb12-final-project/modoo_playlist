...
public class ConversationRepositoryImpl implements ConversationRepositoryCustom {
  ...
  @Override
  public ChatConversationCursorResponse findAiConversations(UUID requesterId,
      SliceCursorRequest request) {
    boolean ascending = isAscending(request);
    int limit = request.limit();
    List<Conversation> rows = queryFactory
        .selectFrom(c)
        .join(c.participants, me)
        .where(
            c.type.eq(ConversationType.AI),
            me.user.id.eq(requesterId),
            cursorCondition(request, ascending)
        )
        .orderBy(
    ...
    Map<UUID, String> firstUserMessages =
        findFirstUserMessages(conversationIds, requesterId);

    List<ChatConversationSummaryDto> data = rows.stream()
        .map(conversation -> new ChatConversationSummaryDto(
            conversation.getId(),
            toTitle(firstUserMessages.get(conversation.getId())),
    ...
    if (hasNext && !rows.isEmpty()) {
      Conversation lastConversation = rows.get(rows.size() - 1);
    ...
    return new ChatConversationCursorResponse(
        data,
        nextCursor,
        nextIdAfter,
        hasNext,
  ...
  private Map<UUID, String> findFirstUserMessages(
    ...
    QMessage first = new QMessage("first");
    List<Tuple> results = queryFactory
        .select(m.conversation.id, m.message)
        .from(m)
        .where(
            m.conversation.id.in(conversationIds),
            m.sender.id.eq(requesterId),
            JPAExpressions
                .selectOne()
                .from(first)
                .where(
                    first.conversation.id.eq(m.conversation.id),
                    first.sender.id.eq(requesterId),
                    first.createdAt.lt(m.createdAt)
                        .or(
                            first.createdAt.eq(m.createdAt)
                                .and(first.id.lt(m.id))
                        )
                )
                .notExists()
        )
    ...
    Map<UUID, String> firstMessages = new HashMap<>();
    for (Tuple result : results) {
      firstMessages.put(
          result.get(m.conversation.id),
          result.get(m.message)
      );
    }
  ...
  private String toTitle(String firstMessage) {
    ...
    return firstMessage.length() <= TITLE_MAX_LENGTH
        ? firstMessage
